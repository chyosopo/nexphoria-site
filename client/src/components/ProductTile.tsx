...
export function ProductTile({ sku }: { sku: SoloPeptide }) {
  ...
  return (
    <Link
      ...
      className="nx-ptile nx-ptile--solid"
      data-testid={`ptile-${sku.slug}`}
      style={{ background: accent.solid, borderColor: accent.solid }}
    >
      <div className="nx-ptile__media">
        <VialHero sku={sku} width="100%" />
        <span className="nx-ptile__cat" style={{ background: accent.solid, color: accent.on }}>
          {sku.category}
        </span>
      </div>

      {/* SOLID BODY. Every colour below comes from the accent's on/onSoft
          roles — nothing falls back to --nx-fg-graphite or --nx-fg-muted,
          which are tuned for the cold-white canvas and go nearly invisible on
          a jewel ground. That was the visible failure in the option C mockup:
          the price note and the safety line vanished into the green. */}
      <div className="nx-ptile__body">
        ...
        <h3 style={{ fontFamily: S, fontWeight: 500, fontSize: "var(--nx-t-xl)", lineHeight: 1.15, color: accent.on }}>
          {sku.outcome}
        </h3>
        <p style={{ fontFamily: F, fontSize: "var(--nx-t-xs)", fontWeight: 600, letterSpacing: "var(--nx-ls-caps)", textTransform: "uppercase", color: accent.onSoft, marginTop: "0.45rem" }}>
          {sku.name}
        </p>

        <p className="nx-ptile__mech" style={{ fontFamily: F, fontSize: "var(--nx-t-sm)", lineHeight: 1.5, color: accent.onSoft, marginTop: "0.7rem" }}>
          {sku.mechanism}
        </p>
        ...
        <ol className="nx-ptile__timeline" aria-label={`What happens on ${sku.name}`}>
          {sku.timeline.map((t) => (
            <li key={t.wk}>
              <span className="nx-ptile__wk" style={{ color: accent.on }}>{t.wk}</span>
              <span className="nx-ptile__eff" style={{ color: accent.onSoft }}>{t.effect}</span>
            </li>
          ))}
        </ol>

        <div className="nx-ptile__foot">
          <div>
            <p style={{ fontFamily: S, fontWeight: 500, fontSize: "var(--nx-t-h3)", color: accent.on, lineHeight: 1 }}>
              {price.figure}
            </p>
            <p style={{ fontFamily: F, fontSize: "var(--nx-t-2xs)", color: accent.onSoft, marginTop: "0.25rem" }}>
              {price.note}
            </p>
          </div>
          {/* The arrow chip inverts — white disc, accent glyph — because an
              accent-on-accent chip disappears on a solid card. */}
          <span className="nx-ptile__go" style={{ background: accent.on, color: accent.solid }}>
        ...
        <p style={{ fontFamily: F, fontSize: "var(--nx-t-2xs)", lineHeight: 1.4, color: accent.onSoft, marginTop: "0.6rem" }}>
