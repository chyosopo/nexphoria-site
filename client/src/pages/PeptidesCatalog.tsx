        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          {cats.map((c) => (
            <button key={c} onClick={() => setFilter(c)} style={{
              fontFamily: F, fontSize: 13, fontWeight: 600, padding: "8px 15px", borderRadius: 999, cursor: "pointer",
          ...
          {shown.map((s, i) => (
            <Reveal key={s.slug} delay={i * 40}>
              <Link href={`${base}/peptides/${s.slug}`} className="nx-glass-tile" data-testid={`peptide-${s.slug}`} style={{ height: "100%", display: "block" }}>
                ...
                <p style={{ fontFamily: F, fontSize: 13, lineHeight: 1.45, color: "var(--nx-fg-graphite)", marginTop: "0.5rem" }}>{s.dose}</p>
